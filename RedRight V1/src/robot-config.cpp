...
motor UpperLeft = motor(PORT18, ratio6_1, true);
...
digital_out ClumpA = digital_out(Brain.ThreeWirePort.A);
digital_out Arm = digital_out(Brain.ThreeWirePort.B);
controller Controller1 = controller(primary);
inertial Inertial = inertial(PORT3);
motor_group RightDrive = motor_group(TopRight,LowerRight);
motor_group LeftDrive = motor_group(TopLeft,LowerLeft);
motor_group Engage = motor_group(Conveyor,Intake);
drivetrain Drivetrain = drivetrain (LeftDrive,RightDrive,320,320,40,mm,.75);
smartdrive SmartDrive = smartdrive (LeftDrive,RightDrive,Inertial,320,320,40,mm,.75);
